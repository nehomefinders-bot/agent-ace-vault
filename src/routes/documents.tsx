...
function Documents() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from("documents")
      .select("id,name,folder,file_path,library_scope,size_bytes,mime_type,created_at,status,signed_at")
      .eq("library_scope", "documents")
  ...
  if (!user) {
    return (
      <PageShell title="Documents" subtitle="Sign in to manage documents.">
  ...
  return (
    <PageShell
      title="Documents"
      subtitle="Every contract, disclosure, and signed document organized in a legal workflow ledger."
      actions={
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {(["all", "pending", "signed"] as const).map((f) => (
          <Button key={f} size="sm" variant={filter === f ? "default" : "outline"} onClick={() => setFilter(f)} className="capitalize">
            {f === "pending" ? "Pending Signature" : f === "signed" ? "Fully Executed" : "All"}
          </Button>
        ))}
      </div>
      {loading ? (
            ...
            <section key={g.folder} className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
              <header className="px-4 sm:px-6 py-3 border-b border-border bg-muted/30">
                <h2 className="text-sm font-semibold inline-flex items-center gap-2">
                  <FolderOpen className="h-4 w-4" /> {g.folder}
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 shrink-0 ml-auto">
                      {d.status === "signed" ? (
                        ...
                        <Button size="sm" variant="outline" onClick={() => setSignDoc(d)}>
                          <PenLine className="h-4 w-4 mr-1" /> Sign and Send
                        </Button>
                      )}
                      <button onClick={() => preview(d)} title="Preview" className="p-2 rounded-md hover:bg-muted text-muted-foreground">
                        <Eye className="h-4 w-4" />
                      </button>
                      <button onClick={() => download(d)} title="Download" className="p-2 rounded-md hover:bg-muted text-muted-foreground">
                        <Download className="h-4 w-4" />
                      </button>
                      <button onClick={() => remove(d)} className="p-2 rounded-md hover:bg-muted text-destructive">
